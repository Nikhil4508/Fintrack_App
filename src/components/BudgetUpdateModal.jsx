...
const BudgetUpdateModal = ({ budget, onUpdate, onClose }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const newBudget = {
      ...budget,
      amt: `$${newSpent.toFixed(2)} / $${total.toFixed(2)}`,
      used: `${newUsage}%`,
      remain: `$${newRemain.toFixed(2)}`
    };
  ...
  return (
    <>
      ...
      <div className="fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border border-[var(--border-color)] bg-[var(--background-color)] p-6 shadow-lg duration-200 sm:rounded-lg sm:max-w-[425px]">
        <form onSubmit={handleSubmit}>
          <div className="flex flex-col space-y-1.5 text-center sm:text-left">
            ...
            <p className="text-sm text-[var(--sub-heading-text)]">
              Add or subtract spending from your {budget.period || "monthly"} {budget.title.toLowerCase()} budget.
            </p>
          </div>
          <div className="grid gap-4 py-4">
            <div className="bg-[var(--sub-hover)] rounded-lg p-4 space-y-2 text-[var(--heading-text)]">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-green-500">${remaining.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="">Usage:</span>
                <span className={`font-medium ${usage > 50 ? "text-red-500":"text-green-500"}`}>{usage}%</span>
              </div>
            </div>
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-x-icon lucide-x w-5 h-5"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
        </button>
      </div>
    </>
  );
