...
const Budgetmodal = ({ onClose, addCategoryBudget }) => {
  ...
  const [selectedPeriodCategory, setSelectedPeriodCategory] =
    useState("Monthly");
            ...
            <div className="space-y-2">
              ...
              <div className="relative" ref={dropdownRef}>
                <button
                  id="category"
                  type="button"
                  className="flex h-10 w-full items-center justify-between rounded-md border border-[var(--border-color)] bg-[var(--background-color)] px-3 py-2 text-sm ring-offset-[var(--background-color)] placeholder:text-[var(--sub-heading-text)] focus:outline-none focus:ring-2 focus:ring-[var(--sub-background-color)] focus:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1 mt-2"
                  onClick={() => setDropdownOpen((open) => !open)}
                >
                  <span className="flex items-center text-[var(--heading-text)] ">
                    {selectedBudgetCategory && (
                      <span className="inline-block mr-2 align-middle ">
                        {categoryIcons[selectedBudgetCategory]}
                      </span>
                    )}
                ...
                {dropdownOpen && (
                  <ul
                    className="absolute z-10 mt-1 w-full bg-[var(--background-color)] border border-[var(--border-color)] rounded-md shadow-lg max-h-60 overflow-y-scroll  [&::-webkit-scrollbar]:w-2
                ...
                [&::-webkit-scrollbar-thumb]:bg-gray-400 [&::-webkit-scrollbar-thumb]:rounded "
                  >
                    {budgetcategories.map((cat) => (
                      <li
                        key={cat}
                        className={`px-4 py-2 cursor-pointer flex text-sm text-[var(--heading-text)] items-center hover:bg-[var(--sub-hover)] ${selectedBudgetCategory === cat ? "bg-[var(--sub-hover)] font-semibold" : ""}`}
                        onClick={() => {
            ...
            <div className="space-y-2">
              ...
              <div className="relative" ref={periodRef}>
                <button
                  id="period"
                  type="button"
                  className="flex h-10 w-full items-center justify-between rounded-md border border-[var(--border-color)] bg-[var(--background-color)] px-3 py-2 text-sm text-[var(--heading-text)] ring-offset-[var(--sub-background-color)] placeholder:text-[var(--sub-heading-text)] focus:outline-none focus:ring-1 focus:ring-[var(--sub-background-color)] focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1 mt-2"
                  onClick={() => setPeriodDropDownOpen((open) => !open)}
                >
                  <span>{selectedPeriodCategory || "Monthly"}</span>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                    height="24"
                ...
                {periodDropDownOpen && (
                  <ul className="absolute z-10 mt-1 w-full bg-[var(--background-color)] border border-[var(--border-color)] rounded-md shadow-lg max-h-60 overflow-auto">
                    {periodCategories.map((cat) => (
                      <li
                        key={cat}
                        className={`px-4 py-2 cursor-pointer flex text-sm items-center text-[var(--heading-text)] hover:bg-[var(--sub-hover)] ${selectedPeriodCategory === cat ? "bg-[var(--sub-hover)] font-semibold" : ""}`}
                        onClick={() => {
