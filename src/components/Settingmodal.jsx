  ...
  const handleImageUpload = (event) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        setImage(imageData);
        localStorage.setItem('profileImage', imageData); // Store image in local storage
    ...
    <>
      ...
      <div className="fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border border-[var(--border-color)] bg-[var(--background-color)] p-6 shadow-lg  sm:rounded-lg sm:max-w-[425px]">
        <div className="flex flex-col space-y-1.5 text-center sm:text-left">
          <h1 className='text-lg text-[var(--heading-text)] font-semibold leading-none tracking-tight'>Update Profile Picture</h1>
          <p className="text-sm text-[var(--sub-heading-text)]">Upload a new profile picture. JPG, PNG, and GIF formats are supported.</p>
        </div>
        <div className="flex flex-col items-center space-y-4 py-4">
          ...
          <div className="grid w-full gap-2">
            <label htmlFor="avatar" className='text-sm text-[var(--heading-text)] font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 sr-only'>
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-upload-icon lucide-upload h-4 w-4"><path d="M12 3v12"/><path d="m17 8-5-5-5 5"/><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/></svg>
              Change Image
            </button>
          </div>
        </div>
        <button type='button' className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none text-[var(--sub-heading-text)]"
        onClick={onClose}
        >
